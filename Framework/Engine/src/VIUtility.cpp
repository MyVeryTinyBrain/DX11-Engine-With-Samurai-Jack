...
VI* VIUtility::CreateNormalizedQuad()
{
	Vertex vertices[4] = {};
	...
	VI* vi = nullptr;
	HRESULT hr = Private::CreateAndSetup(&vi, vertices, 4, triangles, 2);

	return vi;
}

VI* VIUtility::CreateQuad(V2 extents)
{
	Vertex vertices[4] = {};
	TriangleIndex triangles[2] = {};
...
VI* VIUtility::CreatePlane(V2 extents)
{
	Vertex vertices[4] = {};
	TriangleIndex triangles[2] = {};
...
VI* VIUtility::CreateBox(V3 extents)
{
	Vertex vertices[24] = {};
	...
	VI* vi = nullptr;
	HRESULT hr = Private::CreateAndSetup(&vi, vertices, 24, triangles, 12);

	return vi;
}

VI* VIUtility::CreateSkyBox()
{
...
VI* VIUtility::CreateSphere(float radius, uint numV, uint numH)
{
	return CreateCapsule(radius, 0.0f, numV, numH);
}

VI* VIUtility::CreateCapsule(float radius, float halfHeight, uint numV, uint numCapH)
{
	if (numV < 3)
		numV = 3;
	...
	VI* vi = nullptr;
	HRESULT hr = Private::CreateAndSetup(&vi, vertices, numVertices, triangles, numTriangles);
...
VI* VIUtility::CreateCylinder(float radius, float halfHeight, uint numV)
{
	if (numV < 3)
		numV = 3;
...
void VIUtility::SetQuadInScreen(const V2& screenSize, const FRect& rect, VI* vi)
{
	float x = rect.Left;
	float y = rect.Top;
	...
	float dw = screenSize.x * 0.5f;
	float dh = screenSize.y * 0.5f * -1.0f;
...
HRESULT VIUtility::Private::CreateAndSetup(VI** out_vi, Vertex* vertices, uint vCount, TriangleIndex* triangles, uint tCount)
{
	if (!out_vi)
		return E_FAIL;
