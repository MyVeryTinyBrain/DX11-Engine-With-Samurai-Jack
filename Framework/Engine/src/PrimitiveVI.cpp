...
VI* PrimitiveVI::CreateNormalizedQuad()
{
	...
	vertices[1].uv = V2(1, 0);
	vertices[2].uv = V2(1, 1);
	vertices[3].uv = V2(0, 1);
...
VI* PrimitiveVI::CreateBox(V3 extents)
{
	...
	vertices[4].uv = V2(1, 0);
	vertices[5].uv = V2(0, 0);
	vertices[6].uv = V2(0, 1);
	vertices[7].uv = V2(1, 1);
	...
	vertices[8].uv = V2(0, 0);
	vertices[9].uv = V2(1, 0);
	vertices[10].uv = V2(1, 1);
	vertices[11].uv = V2(0, 1);
	...
	vertices[12].uv = V2(1, 0);
	vertices[13].uv = V2(0, 0);
	vertices[14].uv = V2(0, 1);
	vertices[15].uv = V2(1, 1);
	...
	vertices[16].uv = V2(0, 0);
	vertices[17].uv = V2(1, 0);
	vertices[18].uv = V2(1, 1);
	vertices[19].uv = V2(0, 1);
	...
	vertices[20].uv = V2(1, 0);
	vertices[21].uv = V2(0, 0);
	vertices[22].uv = V2(0, 1);
	vertices[23].uv = V2(1, 1);
...
VI* PrimitiveVI::CreateSphere(float radius, uint slice, uint step)
{
	...
	for (uint i = 1; i < step; ++i)
	{
		...
		for (uint j = 0; j < slice; ++j)
		{
			...
			vertices[vIndex].uv = V2(spinPercent, heightPercent);
	...
	for (uint i = 0; i < slice; ++i)
	{
		...
		vertices[topVertexBeginIndex + i].uv = V2(spinPercent, 0);
		vertices[topVertexBeginIndex + i].normal = V3(0, 1, 0);

		vertices[bottomVertexBeginIndex + i].position = V3::down() * radius;
		vertices[bottomVertexBeginIndex + i].uv = V2(spinPercent, 1);
...
VI* PrimitiveVI::CreateCylinder(float radius, float halfHeight, uint slice)
{
	...
	for (uint i = 0; i < slice; ++i)
	{
		...
		vertices[topCapVIdx].uv = vertices[bottomCapVIdx].uv = V2(x + 1.0f, z + 1.0f) * 0.5f;
		vertices[topSideVIdx].uv.x = vertices[bottomSideVIdx].uv.x = spinPercent;
		vertices[topSideVIdx].uv.y = 0;
		vertices[bottomSideVIdx].uv.y = 1;
	...
	vertices[topCenterVIdx].uv = vertices[bottomCenterVIdx].uv = V2(0.5, 0.5f);
