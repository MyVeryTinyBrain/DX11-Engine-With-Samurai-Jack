...
// - For the getter you can testOp (TableGetColumnFlags() & ImGuiTableColumnFlags_IsEnabled) != 0.
...
void ImGui::TableHeader(const char* label)
{
    ...
    {
        // While moving a column it will jump on the other side of the mouse, so we also testOp for MouseDelta.x
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            KeepAliveID(column_id);
            if (IsClippedEx(column_hit_rect, column_id)) // FIXME: Can be removed or replaced with a lower-level testOp
                continue;
