...
namespace ImGui
{
    // ContextStr creation and access
    // - Each context create its own ImFontAtlas by default. You may instance one yourself and pass it to CreateContext() to share a font atlas between contexts.
    // - DLL users: heaps and globals are not shared across DLL boundaries! You will need to call SetCurrentContext() + SetAllocatorFunctions()
    //   for each static/DLL boundary you are calling from. Read "ContextStr and Memory Allocators" section of imgui.cpp for details.
    ...
    IMGUI_API const ImGuiPayload*   GetDragDropPayload();                                                           // peek directly into the current payload from anywhere. may return NULL. use ImGuiPayload::IsDataType() to testOp for the payload type.
    ...
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // testOp if rectangle (of given size, starting from cursor position) is visible / not clipped.
    IMGUI_API bool          IsRectVisible(const ImVec2& rect_min, const ImVec2& rect_max);      // testOp if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
    ...
    // - DLL users: heaps and globals are not shared across DLL boundaries! You will need to call SetCurrentContext() + SetAllocatorFunctions()
    //   for each static/DLL boundary you are calling from. Read "ContextStr and Memory Allocators" section of imgui.cpp for more details.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoMouseInputs          = 1 << 9,   // Disable catching mouse, hovering testOp with pass through.
...
enum ImGuiPopupFlags_
{
    ...
    ImGuiPopupFlags_AnyPopupId              = 1 << 7,   // For IsPopupOpen(): ignore the ImGuiID parameter and testOp for any popup.
    ImGuiPopupFlags_AnyPopupLevel           = 1 << 8,   // For IsPopupOpen(): search/testOp at any level of the popup stack (default testOp in the current level)
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_AcceptBeforeDelivery         = 1 << 10,  // AcceptDragDropPayload() will returns true even before the mouse button is released. You can then call IsDelivery() to testOp if the payload needs to be delivered.
...
// To testOp io.KeyMods (which is a combination of individual fields io.KeyCtrl, io.KeyShift, io.KeyAlt set by user/backend)
