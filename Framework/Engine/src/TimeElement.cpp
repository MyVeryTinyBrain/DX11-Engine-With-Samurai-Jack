...
ProgramTimeElement::ProgramTimeElement() : TimeElement(0)
{
}

unsigned int ProgramTimeElement::Accumulate()
{
    LONGLONG currentTick = GetTick();
    ...
    m_accumulated += elapsed * m_timeScale;

    m_unscaledAccumulated += elapsed;

    return 0;
}

float ProgramTimeElement::GetUnscaledAccumulated() const
{
    return m_unscaledAccumulated;
}
